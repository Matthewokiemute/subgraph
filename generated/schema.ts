...
export class MerkleRootChanged extends Entity {
  constructor(id: Bytes) {
    super();
    this.set("id", Value.fromBytes(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save MerkleRootChanged entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type MerkleRootChanged must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("MerkleRootChanged", id.toBytes().toHexString(), this);
    }
  }

  static loadInBlock(id: Bytes): MerkleRootChanged | null {
    return changetype<MerkleRootChanged | null>(
      store.get_in_block("MerkleRootChanged", id.toHexString())
    );
  }

  static load(id: Bytes): MerkleRootChanged | null {
    return changetype<MerkleRootChanged | null>(
      store.get("MerkleRootChanged", id.toHexString())
    );
  }
